...
# Build NEW_FUNC as a list of lines to avoid escape-sequence issues
# when the string is written into the target .py file.
_BACKSLASHES = "/\\"   # forward-slash + backslash, used in rstrip below
NEW_FUNC = "\n".join([
    ...
    "        size = frames[0].size",
    "    else:",
    f"        video_name = os.path.basename(frame_root.rstrip({repr(_BACKSLASHES)}))",
    "        frame_files = sorted([",
